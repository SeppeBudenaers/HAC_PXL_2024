...
// ==============================================================
`timescale 1ns/1ps
module ImageTransform_control_r_s_axi
#(parameter
...
//------------------------Parameter----------------------
localparam
    ADDR_IMAGERGBA_DATA_0       = 6'h10,
    ADDR_IMAGERGBA_DATA_1       = 6'h14,
    ADDR_IMAGERGBA_CTRL         = 6'h18,
...
always @(posedge ACLK) begin
    if (ACLK_EN) begin
        if (ar_hs) begin
            rdata <= 'b0;
            case (raddr)
                ADDR_IMAGERGBA_DATA_0: begin
                    rdata <= int_imageRGBA[31:0];
                end
                ADDR_IMAGERGBA_DATA_1: begin
                    rdata <= int_imageRGBA[63:32];
                end
...
//------------------------Register logic-----------------
assign imageRGBA       = int_imageRGBA;
assign output_r_offset = int_output_r_offset;
// int_imageRGBA[31:0]
always @(posedge ACLK) begin
    if (ARESET)
        int_imageRGBA[31:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_IMAGERGBA_DATA_0)
            int_imageRGBA[31:0] <= (WDATA[31:0] & wmask) | (int_imageRGBA[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        int_imageRGBA[63:32] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_IMAGERGBA_DATA_1)
            int_imageRGBA[63:32] <= (WDATA[31:0] & wmask) | (int_imageRGBA[63:32] & ~wmask);
    end
end
