...
entity ImageTransform_control_r_s_axi is
generic (
...
end entity ImageTransform_control_r_s_axi;
...
architecture behave of ImageTransform_control_r_s_axi is
    type states is (wridle, wrdata, wrresp, wrreset, rdidle, rddata, rdreset);  -- read and write fsm states
    signal wstate  : states := wrreset;
    signal rstate  : states := rdreset;
    signal wnext, rnext: states;
    constant ADDR_IMAGERGBA_DATA_0       : INTEGER := 16#10#;
    constant ADDR_IMAGERGBA_DATA_1       : INTEGER := 16#14#;
    constant ADDR_IMAGERGBA_CTRL         : INTEGER := 16#18#;
    ...
    -- internal registers
    signal int_imageRGBA       : UNSIGNED(63 downto 0) := (others => '0');
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_IMAGERGBA_DATA_0 =>
                        rdata_data <= RESIZE(int_imageRGBA(31 downto 0), 32);
                    when ADDR_IMAGERGBA_DATA_1 =>
                        rdata_data <= RESIZE(int_imageRGBA(63 downto 32), 32);
...
-- ----------------------- Register logic ----------------
    imageRGBA            <= STD_LOGIC_VECTOR(int_imageRGBA);
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_IMAGERGBA_DATA_0) then
                    int_imageRGBA(31 downto 0) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_imageRGBA(31 downto 0));
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_IMAGERGBA_DATA_1) then
                    int_imageRGBA(63 downto 32) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_imageRGBA(63 downto 32));
                end if;
            end if;
        end if;
    end process;
